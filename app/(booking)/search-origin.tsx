...
export default function SearchOrigin() {
    ...
    const mappedOrigin = origins.filter((item) => item.city.toLocaleLowerCase().search(search.toLocaleLowerCase()) != -1 || item.station.toLocaleLowerCase().search(search.toLocaleLowerCase()) != -1)
    ...
    return (<View style={{ flex: 1, backgroundColor: '#fff' }}>
        <View style={{ ...styles.header, paddingTop: statusBarHeight, position: 'relative' }}>
            ...
            <View style={{ flex: 1 }}>
                <TextInput onChangeText={e => setSearch(e)} placeholder="Search" style={{ fontFamily: 'Poppins-Regular', height: 50, padding: 8, paddingLeft: 40, borderRadius: 16, backgroundColor: '#fff' }} />
        ...
        >
            <View style={{ flexDirection: 'column', padding: 24, backgroundColor: '#fff' }}>
                {mappedOrigin.length > 0 ? mappedOrigin.map((item, index) => (
                    <InputRow key={index}>
                        <TouchableOpacity onPress={() => { setOrigin(item.station); route.back() }} style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
